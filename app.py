...
try:
    ...
    model = load_model('CNN_LSTM_Model_256.h5', 
...
def predict():
    try:
        ...
        if len(ppg) != sample_size or len(ecg) != sample_size:
            logger.error(f"Invalid input size. Expected {sample_size}, got PPG: {len(ppg)}, ECG: {len(ecg)}")
            return jsonify({'error': f'Input data must be {sample_size} samples long'}), 400
